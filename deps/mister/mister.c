...
void mister_draw(video_driver_state_t *video_st, const void *data, unsigned width, unsigned height, size_t pitch)
{
   ...
   if (mode_switch_pending)
   {
      ...
      /* video_driver_set_output_size moved to mister_resize_viewport via vp_resize_pending */
...
*/
   ...
   /* glcore (gl3_t) and vulkan (vk_t) viewport hacks omitted: those structs are
    * private to their respective driver .c files. */
#else
   settings_t *settings  = config_get_ptr();
   struct video_viewport vp = { 0 };
   video_driver_get_viewport_info(&vp);
