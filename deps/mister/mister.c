...
void mister_draw(video_driver_state_t *video_st, const void *data, unsigned width, unsigned height, size_t pitch)
{
   ...
   if (mode_switch_pending)
   {
      ...
      video_driver_set_output_size(mister_mode.width, mister_mode.height);
...
void mister_resize_viewport(video_driver_state_t *video_st)
{
   ...
   /* glcore (gl3_t) viewport hack omitted: gl3_t is private to gfx/drivers/gl3.c.
    * Falls back to no driver-specific hack for glcore. */
#else
   settings_t *settings  = config_get_ptr();
