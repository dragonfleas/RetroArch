...
void mister_draw(video_driver_state_t *video_st, const void *data, unsigned width, unsigned height, size_t pitch)
{
   ...
   if (settings->bools.video_mister_enable && video_st->frame_count > 0 && !blitMister)
   {
      ...
      if (!blitMister && mister_buffer && (!video_driver_cached_frame_is_hw_render() || menu_on) && pitch > 0) //software rendered
      {
      ...
      if (!blitMister && mister_buffer && data == RETRO_HW_FRAME_BUFFER_VALID && video_driver_cached_frame_is_hw_render()) //hardware rendered
      {
         ...
         if (vp.width != mister_width || vp.height != mister_height || video_st->frame_count == 1)
         {
            ...
            settings->video_vp_custom = vp;
            video_driver_set_aspect_ratio();

            video_driver_update_viewport(&vp, false, true, false);
         }
