      ...
      unsigned height, size_t pitch)
{
   ...
   if (settings->bools.video_mister_enable && video_st->frame_count > 0 && !blitMister)
   {
      ...
      if (!blitMister && mister_buffer && frame_cache_data != RETRO_HW_FRAME_BUFFER_VALID && pitch > 0) //software rendered
      {
   ...
   if (!blitMister && mister_buffer && data == RETRO_HW_FRAME_BUFFER_VALID && frame_cache_data) //hardware rendered
   {
